        ...
        if(len(mlp_input_shape)>1):
            ...
            for i, layer_conf in enumerate(layer_configs):
                ...
                if layer_conf == 'dropout' :
                    model.add(tf.keras.layers.Dropout(self.mlp_dropout, name='dropout'))
                else:
...
def trainmodel(model,x_data,y_data,x_val,y_val,callbacks=None):
    return model.fit(x_data,y_data, batch_size, epochs, validation_data =(x_val,y_val),callbacks= callbacks, verbose=0)
